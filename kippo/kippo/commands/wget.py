...
class command_wget(HoneyPotCommand):
    ...
    def download(self, url, fakeoutfile, outputfile, *args, **kwargs):
        try:
            ...
            path = parsed.path or '/'
            # my mod to allow other ports besides 80
            if scheme == 'https':
                #if scheme == 'https' or port != 80:
                time.sleep(7)
                message = 'OpenSSL: error:140770FC:SSL '
                message += 'routines:SSL23_GET_SERVER_HELLO:unknown protocol\n'
                message += 'Unable to establish SSL connection'
